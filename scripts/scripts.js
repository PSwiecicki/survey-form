...
function addValidators() {
  document.getElementById("name").addEventListener("blur", function (field) {
    let msgBox = {
      ...
      wrong:
        "Name should consist first name and last name without any numbers and characters.",
    };
...
function validation(fieldEvent, msgBox, pattern, capitalizationCorrection) {
  ...
  if (pattern.test(fieldValue)) {
    ...
    fieldEvent.target.setCustomValidity("");
  } else {
    ...
    fieldEvent.target.setCustomValidity("Invalid field.");
  }
}
...
function getClearNameString(nameParts) {
  ...
  ret = ret.concat(
    ...
    nameParts[0].substr(1, nameParts[0].length - 1),
    " ",
    nameParts[1][0].toUpperCase(),
    nameParts[1].substr(1, nameParts[1].length - 1)
  );
  return ret;
}
...
function handleSubmit(e) {
  e.preventDefault();
  alert("Thank you for your aplication!");
}
